...
try {
    ...
    // recete tablosundan verileri çek
    $query = $db->query("SELECT * FROM receteler");
...
<h2>Reçeteler Listesi</h2>
    <table class="table">
        <thead>
        <tr>
            <th scope="col">Reçete ID</th>
            <th scope="col">İlaç Kodu</th>
            <th scope="col">Reçete Sahibi Hasta TC</th>
        </tr>
        </thead>
        <tbody>
        <?php foreach ($receteler as $recete): ?>
            <tr>
                ...
                <td><?= $recete['ilac_kodu'] ?></td>
                <td><?= $recete['recete_sahibi_hasta_tc'] ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
