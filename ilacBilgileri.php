...
try {
    // PostgreSQL sorgusu: İlac tablosundan bilgileri çekme
    $query = $db->query("SELECT * FROM ilaclar");
    $ilacListesi = $query->fetchAll(PDO::FETCH_ASSOC);

    if (count($ilacListesi) > 0) {
        echo '<table class="table">
                <thead>
                    <tr>
                        <th scope="col">İlaç Kodu</th>
                        <th scope="col">İlaç Adı</th>
                        <th scope="col">Açıklama</th>
                    </tr>
                </thead>
                <tbody>';
        foreach ($ilacListesi as $ilac) {
            echo '<tr>
                    <td>' . $ilac["ilac_kodu"] . '</td>
                    <td>' . $ilac["ilac_adi"] . '</td>
                    <td>' . $ilac["aciklama"] . '</td>
                  </tr>';
        }
        ...
        echo '<p>Henüz ilaç bilgisi bulunmamaktadır.</p>';
    }
